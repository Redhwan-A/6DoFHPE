...
class Pose_AGORA_CMU(Dataset):
    # Head pose from AGORA or CMU dataset
    def __init__(self, data_dir, filename_path, transform, image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform

        with open(filename_path) as f:
            lines = f.read().splitlines()
        filename_list, pose_list = [], []
        for line in lines:
            line = line.strip()
            [filename, pitch, yaw, roll] = line.split(" ")
            filename_list.append(os.path.join(self.data_dir, filename))
            pose_list.append([float(pitch), float(yaw), float(roll)])

        self.X_train = filename_list
        self.y_train = pose_list
        self.image_mode = image_mode
        self.length = len(filename_list)
    ...
    def __getitem__(self, index):
        ...
        img = img.convert(self.image_mode)
        pose = self.y_train[index]

        [pitch, yaw, roll] = pose
        # We get the pose in radians, not degrees
...
def getDataset(dataset, data_dir, filename_list, transformations, train_mode = True):
    if dataset == 'Pose_300W_LP':
        ...
        pose_dataset = AFW(
            data_dir, filename_list, transformations)
    elif dataset == 'CMU' or dataset == 'AGORA':
        pose_dataset = Pose_AGORA_CMU(data_dir, filename_list, transformations)
    else:
